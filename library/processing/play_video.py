...
def speak(all_subtitles_list, translation_language):
    ...
    print("jeg er nu i speak -- playvideo linje 28")

    #audio.speak(len(all_subtitles_list))
    pygame.mixer.music.load('data/output/audio/output'+str(len(all_subtitles_list))+'.mp3')#    str(numOfSubLines)+'.mp3')
    print("jeg har loaded lydfilen")
    pygame.mixer.music.play()
    print("jeg har spillet lyden")
...
def capture_video(translation_language, max_frame):
    ...
    print("Frames pr. second: " + str(cap.get(cv2.CAP_PROP_FPS)))
    frames_pr_second = cap.get(cv2.CAP_PROP_FPS)
    count_frames = 0 
